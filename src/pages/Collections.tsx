    ...
    firstBatch.forEach((imageObj: any) => {
      if (imageObj.mediaType === 'video') {
        ...
        video.onerror = () => {
          setMeasuredDims(prev => ({
            ...prev,
            [imageObj.src]: {
              width: Number(imageObj.width) || 1920,
              height: Number(imageObj.height) || 1080
            }
          }));
          setLoadedImages(prev => new Set([...prev, imageObj.src]));
        };
      ...
      warmImg.onload = () => {
        setMeasuredDims(prev => ({
          ...prev,
          [imageObj.src]: {
            width: warmImg.naturalWidth || Number(imageObj.width) || 800,
            height: warmImg.naturalHeight || Number(imageObj.height) || 1000
          }
        }));
        setLoadedImages(prev => new Set([...prev, imageObj.src]));
      };
      warmImg.onerror = () => {
        setMeasuredDims(prev => ({
          ...prev,
          [imageObj.src]: {
            width: Number(imageObj.width) || 800,
            height: Number(imageObj.height) || 1000
          }
        }));
        setLoadedImages(prev => new Set([...prev, imageObj.src]));
      };
                ...
                >
                  <div className="text-center">
                    ...
                    <button
                      ...
                      className="relative overflow-hidden w-full py-2.5 sm:py-3.5 px-3 sm:px-4 rounded-xl text-sm sm:text-base font-bold transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed bg-primary text-primary-foreground hover:shadow-lg hover:scale-[1.02]"
                    >
